...
class Alphabet extends FlxSpriteGroup
{
	...
	public var isJukebox:Bool = false;
	...
	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = false, typed:Bool = false, ?typingSpeed:Float = 0.05, ?textSize:Float = 1, ?isJukeboxy:Bool = false)
	{
		super(x, y);
		forceX = Math.NEGATIVE_INFINITY;
		this.textSize = textSize;

		isJukebox = isJukeboxy;
	...
	override function update(elapsed:Float)
	{
		if (isMenuItem)
		{
			...
			if(forceX != Math.NEGATIVE_INFINITY) {
				...
				if (isJukebox) x = FlxMath.lerp(x, (targetY * targetY * -20) + 130 + xAdd, lerpVal);
				else x = FlxMath.lerp(x, (targetY * 20) + 90 + xAdd, lerpVal);
			}
		}
...
class AlphaCharacter extends FlxSprite
{
	...
	{
		animation.addByPrefix(letter, letter.toUpperCase() + " bold", 0);
	...
	{
		animation.addByPrefix(letter, "bold" + letter, 0);
		animation.play(letter);
		updateHitbox();
	}
	...
	public function createBoldSymbol(letter:String)
	{
		...
		{
			case '.':
				animation.addByPrefix(letter, 'PERIOD bold', 0);
			case "'":
				animation.addByPrefix(letter, 'APOSTRAPHIE bold', 0);
			case "?":
				animation.addByPrefix(letter, 'QUESTION MARK bold', 0);
			case "!":
				animation.addByPrefix(letter, 'EXCLAMATION POINT bold', 0);
			case "(":
				animation.addByPrefix(letter, 'bold (', 0);
			case ")":
				animation.addByPrefix(letter, 'bold )', 0);
			default:
				animation.addByPrefix(letter, 'bold ' + letter, 0);
	...
	public function createLetter(letter:String):Void
	{
		...
		animation.addByPrefix(letter, letter + " " + letterCase, 0);
		animation.play(letter);
		updateHitbox();
	...
	public function createSymbol(letter:String)
	{
		...
		{
			case '#':
				animation.addByPrefix(letter, 'hashtag', 0);
			case '.':
				animation.addByPrefix(letter, 'period', 0);
			case "'":
				animation.addByPrefix(letter, 'apostraphie', 0);
				y -= 50;
			case "?":
				animation.addByPrefix(letter, 'question mark', 0);
			case "!":
				animation.addByPrefix(letter, 'exclamation point', 0);
			case ",":
				animation.addByPrefix(letter, 'comma', 0);
			default:
				animation.addByPrefix(letter, letter, 0);
		}
		animation.play(letter);
