...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (i in 0...WeekData.weeksList.length) {
			...
			for (song in leWeek.songs) {
				...
				addSong(song[0], i, song[1], FlxColor.fromRGB(colors[0], colors[1], colors[2], 200));
			}
		}

		for (i in 0...WeekData.weeksBList.length) {
			trace("checking html5");
		...
		restaurantFilter.color = 0xffffffff;
		FlxTween.color(restaurantFilter, 1.5, 0xff434566, 0xff434566);

		var banner:FlxSprite;
		banner = new FlxSprite().loadGraphic(Paths.image('freeplaybanner'));
		...
		restaurantFilter.color = songs[curSelected].color;
		intendedColor = restaurantFilter.color;
		changeSelection();
		changeDiff();
	...
	override function update(elapsed:Float)
	{
		...
		if(space && instPlaying != curSelected)
		{
			...
			if(curDifficulty == 3)
			{
				...
				FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song, true), 0.7);
				vocals.play();
				vocals.persist = true;
				vocals.looped = true;
				vocals.volume = 0.7;
	...
	function changeSelection(change:Int = 0)
	{
		...
		if(newColor != intendedColor) {
			...
			colorTween = FlxTween.color(restaurantFilter, 1, restaurantFilter.color, intendedColor, {
