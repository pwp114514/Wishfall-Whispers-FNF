...
class StoryMenuBState extends MusicBeatState
{
	...
	// defaults to True
	public static var weekBCompleted:Map<String, Bool> = new Map<String, Bool>();
	...
	override function create()
	{
		...
		// var bg:BGSprite = new BGSprite('covemid', -600,-200, 1, 1);
		// bg.setGraphicSize(Std.int(bg.width * 1.1));
		// add(bg);

		// coveBg = new BGSprite('covemid2', -600, -200, 1, 1);
		// coveBg.setGraphicSize(Std.int(coveBg.width * 1.1));
		// coveBg.alpha = 1;
		// add(coveBg);

		// coveBgMorning = new BGSprite('covemidalt', -600, -200, 1, 1);
		// coveBgMorning.setGraphicSize(Std.int(coveBgMorning.width * 1.1));
		// coveBgMorning.alpha = 1;
		// add(coveBgMorning);
		
		// var blackbox:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('menu_block_right'));
		// blackbox.scrollFactor.set(0, 0);
		// blackbox.setGraphicSize(Std.int(blackbox.width * 0.667));
		...
		FlxTween.color(restaurantFilter, 1, 0x00b395dc, 0x00f1a9ff);
		//FlxTween.color(restaurantFilter, 1, 0x00ffffff, 0xfff1a9ff);
		...
		//for (char in 0...1)
		//{
			var weekCharacterThing:MenuCharacter = new MenuCharacter((FlxG.width * 0.5) * (1) - 150, 'lilac', true);
		...
		for (i in 0...WeekData.weeksBList.length) {
			var sprWeek:FlxSprite = new FlxSprite(leftArrow.x + 35, 55).loadGraphic(Paths.image('storymenu/' + WeekData.weeksBList[i].toLowerCase()));
		...
		//var charArray:Array<String> = WeekData.weeksBLoaded.get(WeekData.weeksBList[0]).weekCharacters;
		for (i in 0...WeekData.weeksBList.length) {
			var sprIcon:FlxSprite = new FlxSprite(leftArrow.x + 35, 200).loadGraphic(Paths.image('menuicons/' + 'icon-' + WeekData.weeksBLoaded.get(WeekData.weeksBList[i]).weekCharacters[0]));
		...
		for (i in 0...WeekData.weeksBList.length)
			{
				WeekData.setDirectoryFromWeek(WeekData.weeksBLoaded.get(WeekData.weeksBList[i]));
				var weekThing:MenuItem = new MenuItem(20, bgSprite.y, WeekData.weeksBList[i]);
	...
	override function update(elapsed:Float)
	{
		...
		difficultySelectors.visible = !weekIsLocked(curWeekB);
		...
		if (!movedBack && !selectedWeek)
		{
			...
			if (controls.UI_UP_P && WeekData.weeksBList.length > 1)
			{
				changeWeek(-1);
				FlxG.sound.play(Paths.sound('scrollMenu'));
			}

			if (controls.UI_DOWN_P && WeekData.weeksBList.length > 1)
			{
				changeWeek(1);
				FlxG.sound.play(Paths.sound('scrollMenu'));
			}
	...
	function selectWeek()
	{
		if (!weekIsLocked(curWeekB))
		{
			if (stopspamming == false)
			{
				FlxG.sound.play(Paths.sound('confirmMenu'));

				grpWeekText.members[curWeekB].startFlashing();
			...
			var leWeek:Array<Dynamic> = WeekData.weeksBLoaded.get(WeekData.weeksBList[curWeekB]).songs;
			...
			PlayState.storyWeek = curWeekB;
			trace(curWeekB + " is the number of the week that is now playing");
	...
	function changeDifficulty(change:Int = 0):Void
	{
		...
		if (curDifficulty < 0)
			{
				...
				FlxTween.color(restaurantFilter, 1, restaurantFilter.color, 0xff56456d);
				FlxTween.tween(scrollDiamonds, {alpha: 0.1}, 1);
			}
		if (curDifficulty >= CoolUtil.difficultyStuff.length)
			{
				curDifficulty = 0;
				FlxTween.color(restaurantFilter, 1, restaurantFilter.color, 0x00f1a9ff);
		...
		if (curDifficulty == CoolUtil.difficultyStuff.length - 1)
		{
			if(willFlash) 
				{
					//lFlxG.camera.zoom = 1.5;
					//FlxG.sound.play(Paths.sound('lucid'));
					FlxG.sound.play(Paths.sound('lucid'), 0.7);
					willFlash = false;
					FlxTween.color(lucidFlash, 1, 0x98ead9ff, 0x00f1a9ff);
				}
			FlxTween.color(restaurantFilter, 1, restaurantFilter.color, 0xff56456d);
			FlxTween.tween(scrollDiamonds, {alpha: 0.1}, 1);
		}
		// else 
		// {
		// 	FlxTween.color(restaurantFilter, 1, 0x00f1a9ff, 0x00f1a9ff);
		// 	FlxTween.tween(bgDiamonds, {alpha: 0.05}, 1);
		// }
		...
		#if !switch
		intendedScore = Highscore.getWeekScore(WeekData.weeksBList[curWeekB], curDifficulty);
		#end
	}
	...
	{ 
		...
		if (curWeekB >= WeekData.weeksBList.length)
			curWeekB = 0;
		if (curWeekB < 0)
			curWeekB = WeekData.weeksBList.length - 1;
		
		var leWeek:WeekData = WeekData.weeksBLoaded.get(WeekData.weeksBList[curWeekB]);
		WeekData.setDirectoryFromWeek(leWeek);

		// if (curWeekB < 0)
		// 	curWeekB = CoolUtil.difficultyStuff.length-1;
		// if (curWeekB >= CoolUtil.difficultyStuff.length)
		// 	curDifficulty = 0;
		trace(WeekData.weeksBList[curWeekB].toLowerCase());
		...
		// for (i in 0...leWeek.weekColor) {
		// 	colors.push(leWeek.weekColor[i][0]);
		// }
		...
		// 	}
			...
			// 	if(colors == null || color.length < 3) {
			// 		colors = [146, 113, 253];
			// 	}
			// 	//ddSong(song[0], i, song[1], FlxColor.fromRGB(colors[0], colors[1], colors[2]));
			// }

		//FlxColor.fromRGB(leWeek.freeplayColor);
		if (true == true)
			{
				var funnyInt:Int;

		// for (number in leWeek.weekColor) {
		// 	var colors:Array<Int> = number[0];
		...
		// txtWeekDesc.x = funnyTextOffset - txtWeekDesc.width/2;
		// txtWeekDesc.y = 135;
		...
		{
			item.targetY = bullShit - curWeekB;
			if (item.targetY == Std.int(0) && !weekIsLocked(curWeekB))
				{
					item.alpha = 1;
	...
	function weekIsLocked(weekNum:Int) {
		//var leWeek:WeekData = WeekData.weeksBLoaded.get(WeekData.weeksBList[weekNum]);
		var leWeek:WeekData = WeekData.weeksBLoaded.get(WeekData.weeksBList[weekNum]);
		return (!leWeek.startUnlocked && leWeek.weekBefore.length > 0 && (!weekBCompleted.exists(leWeek.weekBefore) || !weekBCompleted.get(leWeek.weekBefore)));
	}

	function updateText()
	{
		var weekArray:Array<String> = WeekData.weeksBLoaded.get(WeekData.weeksBList[curWeekB]).weekCharacters;
		...
		//txtTracklist.x = FlxG.width - (txtTracklist.width + 430);
		//txtTracklist.x = 100;
