...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Blammed Lights', "Value 1: Input a hex color code. (Example: 0xff31a2fd = Blue)\n0 = Turn off."],
		['Kill Henchmen', "For Mom's songs, don't use this please, i love them :("],
		['Add Camera Zoom', "Zooms the camera\nValue 1: Camera zoom amount \nValue 2: Zoom attack, aka how fast in seconds it reaches Value 1\nValue 3: How long the zoom is held"],
		['Camera Zoom Release', "Changes the factor at which camera returns to default position.\nValue 1: Larger number decreases return time (Default: 3.125)\nLeave the value blank if you want to use Default."],
		...
		['Flash', "Plays a flashing animation, \n\nValue 1: Hex color code \nFlash length in seconds"],
		['Note Speed Multiplier', "Applies a global note speed multiplier,\n\nValue 1: Adds a multplier; typing 100 resets multiplier\nto default, typing 200 doubles the speed"]
	];
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Events', function()
		{
			...
			var lucidEvent:String = '';
			if(lucid && !MusicBeatState.glutoMode)				
				{
			...
			#end
			{
				...
				clearEvents();
				var events:SwagSong = Song.loadFromJson('events' + lucidEvent, songName);
				trace(lucidEvent);
				for (sec in 0...events.notes.length) {
					for (noteId in 0...events.notes[sec].sectionNotes.length) {
						...
						if(note != null && note[1] < 0) {
							_song.notes[sec].sectionNotes.push([note[0], note[1], note[2], note[3], note[4], note[5]]);
						}
					}
				}
		...
		var stepperBPM:FlxUINumericStepper = new FlxUINumericStepper(10, 57, 1, 1, 1, 339, 1);
		...
		var animSpeedMultiplier:FlxUINumericStepper = new FlxUINumericStepper(10, stepperSpeed.y + 30, 0.5, 1, 0.5, 1, 1);
		animSpeedMultiplier.value = _song.animSpeedMultiplier;
		animSpeedMultiplier.name = 'song_animSpeedMultiplier';
		#if MODS_ALLOWED
		...
		var player1DropDown = new FlxUIDropDownMenuCustom(10, animSpeedMultiplier.y + 30, FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), function(character:String)
		{
		...
		var reloadNotesButton:FlxButton = new FlxButton(noteSplashesInputText.x + 5, noteSplashesInputText.y + 30, 'Change Notes', function() {
		...
		tab_group_song.add(animSpeedMultiplier);
		tab_group_song.add(reloadNotesButton);
		tab_group_song.add(noteSkinInputText);
		tab_group_song.add(noteSplashesInputText);
		tab_group_song.add(new FlxText(stepperBPM.x, stepperBPM.y - 15, 0, 'Song BPM:'));
		tab_group_song.add(new FlxText(stepperSpeed.x, stepperSpeed.y - 15, 0, 'Song Speed:'));
		tab_group_song.add(new FlxText(animSpeedMultiplier.x, animSpeedMultiplier.y - 15, 0, 'Smooth Animation Speed Multiplier:'));
	...
	function loadSong():Void
	{
		...
		var file:Dynamic = Paths.voices(currentSongName, lucid);
		vocals = new FlxSound();
	...
	function generateSong() {
		FlxG.sound.playMusic(Paths.inst(currentSongName, lucid), 1, false);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (wname == 'song_animSpeedMultiplier')
				{
					_song.animSpeedMultiplier = nums.value;
				}
	...
	function loadAudioBuffer() {
		...
		if(FileSystem.exists(Paths.modFolders('songs/' + currentSongName + '/Inst.ogg'))) {
			...
			var leVocals:Dynamic = Paths.inst(currentSongName, lucid);
		...
		if(FileSystem.exists(Paths.modFolders('songs/' + currentSongName + '/Voices.ogg'))) {
			...
			var leVocals:Dynamic = Paths.voices(currentSongName, lucid);
	...
	private function saveEvents()
	{
		...
		if ((data != null) && (data.length > 0))
		{
			...
			if(PlayState.isLucid)
				_file.save(data.trim(), "events-lucid.json");
			else _file.save(data.trim(), "events.json");
		}
	}
